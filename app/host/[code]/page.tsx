...
export default function HostGamePage() {
  ...
  useEffect(() => {
    ...
    const pollVotes = async () => {
      const res = await fetch(
        `/api/game/votes?game_id=${game.id}&round=${currentRound}&matchup_idx=${currentMatchupIdx}`
      )
      const data = await res.json()
      setVotes(data.votes || [])
    }
  ...
  const handleAdvance = async () => {
    ...
    // Always fetch the latest game state after advancing
    const res = await fetch(`/api/game/by_code?code=${code}`)
    const data = await res.json()
    setGame(data.game)
    setBracket(data.game?.bracket || [])
  ...
  // Helper to resolve a track ID to the full track object
  const getTrackById = (id: string | null) => {
    if (!id || !game?.tracks) return null
    return game.tracks.find((t: any) => t.id === id) || null
  }
  ...
  const matchup = bracket?.rounds?.[currentRound]?.matches?.[currentMatchupIdx]
  if (!matchup) return <div className="p-8 text-center">No matchup found.</div>

  // Resolve tracks for the current matchup
  const trackA = getTrackById(matchup.trackA_id)
  const trackB = getTrackById(matchup.trackB_id)
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="text-lg font-semibold text-gray-900 mt-4">
          {winner.name} by {winner.artists?.map((a: any) => a.name).join(", ")}
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
      ...
      <div className="w-full max-w-4xl mb-8">
        ...
        <div className="flex flex-col gap-4">
          {bracket.rounds?.map((round: any, roundIdx: number) => (
            <div key={roundIdx} className="border-b pb-2 mb-2">
              <div className="font-semibold mb-1">
                Round {round.round_number}
              </div>
              <div className="flex flex-wrap gap-4">
                {round.matches.map((matchup: any, matchupIdx: number) => {
                  const trackA = getTrackById(matchup.trackA_id)
                  const trackB = getTrackById(matchup.trackB_id)
                  const winnerTrack = getTrackById(matchup.winner_track_id)
                  return (
                    <div
                      key={matchupIdx}
                    ...
                    >
                      <div>
                        <span className="font-medium">
                          {trackA ? trackA.name : "(bye)"}
                        </span>
                        {" vs "}
                        <span className="font-medium">
                          {trackB ? trackB.name : "(bye)"}
                        </span>
                      </div>
                      {winnerTrack && (
                        <div className="text-green-700 text-xs mt-1">
                          Winner: {winnerTrack.name}
                        </div>
                      )}
                      <div className="text-xs text-gray-500 mt-1">
                        Votes: {matchup.votes_trackA} - {matchup.votes_trackB}
                      </div>
                    </div>
                  )
                })}
      ...
      <h3 className="text-lg font-semibold mb-2">
        Round {currentRound + 1} - Matchup {currentMatchupIdx + 1}
      </h3>
      <div className="flex flex-row gap-12 justify-center items-center">
        {[trackA, trackB].map((track, idx) =>
          track ? (
            <div key={track.id} className="flex flex-col items-center">
              ...
              <div className="mt-2 text-xs text-gray-500">
                Total votes: {(votesByTrack[track.id] || []).length}
              </div>
            </div>
          ) : null
        )}
      ...
      <div className="mt-8">
        ...
        <ul className="text-sm text-gray-700">
          {[...new Map(users.map((u: any) => [u.user_id, u])).values()].map(
            (u: any) => (
